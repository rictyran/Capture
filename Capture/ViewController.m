...
- (void)viewDidLoad {
    ...
    self.cameraPickerController.mediaTypes = @[(NSString *)kUTTypeImage,(NSString *)kUTTypeVideo];
    
    self.cameraPickerController.view.frame = self.cameraView.bounds;
    
    [self.cameraView addSubview:self.cameraPickerController.view];
    
    [self.deviceChoiceButton addTarget:self.cameraPickerController action:@selector(changeDeviceType) forControlEvents:UIControlEventTouchUpInside];
...
-(void)changeDeviceType {
    ...
    switch (self.deviceChoiceButton.selectedSegmentIndex) {
